...
{
    public class UpgradeCritValue : MonoBehaviour
    {
        ...
        public void UpdatePrefab()
        {
            if (_headerText == null)
                LoadComponentsForPrefab();
            _headerText.text = _localizationSystem.TranslateWord("UPGRADE_HEADER_VALUE_CRIT");
            _levelText.text = "lvl:" + _playerData.UpgradesData.UpgradeValueCritLevel;
            _bodyText.text = _localizationSystem.TranslateWord("UPGRADE_BODY_VALUE_CRIT");
            _costText.text = _donutConvertSystem.ConvertNumber(_playerData.UpgradesData.UpgradeValueCritCost) +
                             "<sprite=0>";

            _button.interactable = !(_playerData.UpgradesData.UpgradeValueCritCost > _playerData.Donut);
            _textButton.text = _localizationSystem.TranslateWord(_textButton.name);
        }

        private void BuyCritValueItem()
        {
            if (!_playerDataManager.DelDonuts(_playerData.UpgradesData.UpgradeValueCritCost))
                return;

            _playerDataManager.AddValueCrit();
            _upgradesDataManager.AddCritValueLevel();
            _upgradesDataManager.SetValueCritCost(_playerData.UpgradesData.UpgradeValueCritCost * 100_000);
            _stats.AddBuyUpgradesInShop();
            _stats.AddSpentWithUpgrade(_playerData.UpgradesData.UpgradeValueCritCost);
            UpdatePrefab();
        }
